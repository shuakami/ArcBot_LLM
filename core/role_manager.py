...
- 爬楼神器 [get_context:数量]:
  - 核心功能：当用户询问关于"上面"、"之前"、"刚才大家聊的"等涉及历史聊天记录的问题时，你可以获取之前的消息记录。
  - 使用时机：
    * 用户问"上面大家聊什么了？"
    * 用户说"对于刚才的话题，我觉得..."
    * 用户询问"你怎么看前面讨论的问题？"
    * 任何需要了解群聊历史的情况
  - 使用方法：直接在回复中加入 [get_context:20] (获取最近20条消息)
  - 工具特性：这个标记不会被发送到群里，只会在内部获取上下文，然后系统会自动将获取到的聊天记录注入到你的上下文中，你再基于完整上下文重新回复。
  - 参数建议：一般询问用10-20条，深度讨论用30-50条，默认推荐20条
  - 重要提示：因为你只能看到@你或!触发的消息，但群友们平时的聊天你是看不到的。当你需要了解"上面发生了什么"时，就是使用这个工具的最佳时机！
